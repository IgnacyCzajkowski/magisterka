...
network_names = ["email", "infectious", "ucirving"]
...
fig_prec, axs_prec = plt.subplots(3, 3)
fig_rank, axs_rank = plt.subplots(3, 3)
            ...
            for line in file:
                ...
                for n in range(inf_num):
                    try:
                        ...
                        print(beta_name + " " + network_name + " " + str(n) + str(inf_num))
                        print(line.split(" "))    
...
for x, axs in enumerate([axs_prec, axs_rank]):
    ...
    for i in range(len(beta_labels)):
        rows_lims.append((max(min(axs[i, 0].get_ylim()[0], axs[i, 1].get_ylim()[0], axs[i, 2].get_ylim()[0]), 0), max(axs[i, 0].get_ylim()[1], axs[i, 1].get_ylim()[1], axs[i, 2].get_ylim()[1])))
    for i, ax in enumerate(axs.flat):
        ...
        if x == 0:
            ...
            ax.set_yticks(np.arange(round(rows_lims[i // 3][0], 2), round(rows_lims[i // 3][1], 2) + 0.001, step=round((rows_lims[i // 3][1] - rows_lims[i // 3][0]) / 5, 2)))
        else:
            ax.set_yticks(np.arange(round(rows_lims[i // 3][0]), round(rows_lims[i // 3][1]) + 0.001, step=round((rows_lims[i // 3][1] - rows_lims[i // 3][0]) / 5)))    
            #ax.set_ylim([0,1])
        '''    
        else:
    ...
    axs[0, 0].set_title(label="Email")
    axs[0, 1].set_title(label="Infectious")
    axs[0, 2].set_title(label="Ucirving")

for i in range(len(beta_labels)):
    print(axs_prec[i, 1].get_ylim())
    axs_prec[i, 2].text(2.15, axs_prec[i, 2].get_ylim()[0] + 0.5 * (axs_prec[i, 2].get_ylim()[1] - axs_prec[i, 2].get_ylim()[0]), beta_labels[i]) 
    axs_rank[i, 2].text(2.15, axs_rank[i, 2].get_ylim()[0] + 0.5 * (axs_rank[i, 2].get_ylim()[1] - axs_rank[i, 2].get_ylim()[0]), beta_labels[i])

fig_prec.savefig(f"ilosc_inf_prec_real_{sys.argv[1]}.pdf") 
fig_rank.savefig(f"ilosc_inf_rank_real_{sys.argv[1]}.pdf")